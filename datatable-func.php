<?php
    ...
    if(count($col_unique_filters_selected_keys) > 0) {
        ...
        for($i = 0; $i < count($col_unique_filters_selected_keys); $i++) {
            if(array_search($col_unique_filters_selected_keys[$i], $qColumns, true) == false) continue;
            $qWhere .= ($first ? "" : " AND ") . explode(' as ', $col_unique_filters_selected_keys[$i])[0] . " LIKE '%{$col_unique_filters_selected_values[$i]}%'";
            $first = false;
        }
        $qWhere .= ")";
    }
    ...
    if(count($col_select_filter) > 0) {
        for($i = 0; $i < count($col_select_filter); $i++) {
            if(array_search($col_select_filter[$i], $qColumns, true) === false) {
                unset($col_select_filter[$i]);
                continue;
            }
            $qUniqueCols .= ", (SELECT GROUP_CONCAT(DISTINCT($col_select_filter[$i]) SEPARATOR '|') FROM $qFrom) as unique_$col_select_filter[$i]";
        }
    }
